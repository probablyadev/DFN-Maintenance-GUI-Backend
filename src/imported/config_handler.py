...
def load_config(stationfilename):
    ...
    if not 'hostname' in conf_dict['station']:
        if 'name' in conf_dict['station']:
			...
			dum_name = dfn.name_from_path(os.path.dirname(stationfilename))
            if dum_name != 'DFNUNKNOWN':
                logger.warning('config_hostname_from path, ' + dum_name)
                conf_dict['station']['hostname'] = dum_name
            else:
                logger.warning('config_hostname_not_found_using_host, ' + dfn.name())
                conf_dict['station']['hostname'] = dfn.name()
